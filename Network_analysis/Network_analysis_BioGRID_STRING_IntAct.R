Network_analysis <- function(working_dir,Results_dir,BioGRID_data_file,STRING_data_file,IntAct_data_file,file_DEA_names,phenotype_names,phenotype_comparison,splicing_file_name){
  ...
  get_protein_idMART <- function(names,mart){
    if(length(names)>0){
      aux2 <- getBM(filters = "uniprot_gn_id",
                    attributes = c("entrezgene_id", "uniprot_gn_id"),
                    values = as.character(names), mart = mart)
  ...
  extract_edges_NONDIRECTIONAL <- function(data_ini,Lookup_data){
    ...
    ###THIS PART DESTROYS ANY DIRECTIONALITY######################################################## Remove it to make a directional edge table
    for (j in 1:nrow(aux))
    {
  ...
  centrality_extraxtion <- function (aux,save_name,i,mart,phenotype_comparison,Transcriptomics_DTU_total,flag,multi_omics_data,enhancement_flag,file_DEA_names_short){
    ...
    if ((length(which(is.na(aux)))==length(aux)) == FALSE){
      ...
      if(max(lengths(aux_decomposed))>10){
        ...
        #Remove very lengthy calculus of centralities - do them afterwards with all the cores if necessary
        ...
        axx <- getBM(filters = "entrezgene_id",
                     attributes = c("entrezgene_id", "external_gene_name"),
                     values = as.character(rownames(centralities_values_CINNA1)), mart = mart)
		...
		centralities_values_CINNA1 <- cbind(centralities_values_CINNA1,vv1)
        ...
        xlsx::write.xlsx(centralities_values_CINNA1,file=save_name1,sheetName = phenotype_comparison[i],col.names=TRUE,row.names=TRUE,append = TRUE)
      }
    }
  }
  ...
  add_splicing_inforamtion <- function(i,symmetry_value,data_DTU,file_DEA_edges,data_aux_total,splicing_file_index){
    ...
    for (k in 1:length(file_DEA_names)){
      if(k != splicing_file_index){
        aux_total_aux2 <- as.matrix(rbind(aux_total_aux1,as.matrix(file_DEA_edges[[k]][[factor+symmetry_value]])))
      }
    }
  ...
  #file_phospho = file_phospho_name
  #file_proteomics = file_proteomics_name
  #file_transcriptomics_genes = file_transcriptomics_genes_name
  #file_transcriptomics_DTU = file_transcriptomics_DTU_name
  #file_TranscriptionFactors = file_TranscriptionFactors_name
