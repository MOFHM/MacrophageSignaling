function [] = plot_generation_onlyplots(final_data_p,final_data_p_zscore,final_data_p_qnorm,int_data,header,header_p,p_names_p,position_protein_p,nr_rep,nr_miss_total,Colors,sample_names,nr_sample_groups,dir)
...
%Create the correlation plot of the z transfored data
for j = 1:size(final_data_p_zscore,2)
    corrplot(final_data_p_zscore{j},'type','Spearman','tail','both','testR','on','varNames',header_p{j})
    WindowAPI(gcf,'maximize')
    exportgraphics(gcf,strcat(dir,'/Corr_plot_zscore_p',num2str(j),'.pdf'),'ContentType','vector')
end
close all

%Create the correlation plot of the quantile normalized data
for j = 1:size(final_data_p_qnorm,2)
    corrplot(final_data_p_qnorm{j},'type','Spearman','tail','both','testR','on','varNames',header_p{j})
    WindowAPI(gcf,'maximize')
    exportgraphics(gcf,strcat(dir,'/Corr_plot_qnorm_p',num2str(j),'.pdf'),'ContentType','vector')
end
close all

%Create the clustergram for the z transformed data
for j = 1:size(final_data_p_zscore,2)
    aux = final_data_p_zscore{j};
    aux(find(isnan(aux))) = 0;
    aux = impute_data_normal(aux);
    clustergram(aux,'RowLabels',p_names_p{j},'ColumnLabels',header_p{j},'ImputeFun',@knnimpute,'ColumnLabelsRotate',45,'Standardize','column')
    exportgraphics(gcf,strcat(dir,'/Clustergram_plot_zscore_p',num2str(j),'.pdf'),'ContentType','vector')
end
close all

%Create the clustergram for the quantile normalized data
for j = 1:size(final_data_p_qnorm,2)
    aux = final_data_p_qnorm{j};
    aux(find(isnan(aux))) = 0;
    aux = impute_data_normal(aux);
    clustergram(aux,'RowLabels',p_names_p{j},'ColumnLabels',header_p{j},'ImputeFun',@knnimpute,'ColumnLabelsRotate',45,'Standardize','column')
...
%Create the correlation heatmap for the z transformed data
for j = 1:size(final_data_p_zscore,2)
    aux = final_data_p_zscore{j};
    aux = impute_data_normal(aux);
    ...
    %colormap(jet)
    colorbar('EastOutside')
    set(gca,'XTick',1:size(final_data_p_zscore{j},2),'XTickLabels',header_p{j})
    xtickangle(45)
    ylabel('Phosphopeptide number')
    xlabel('Experiment ID')
    title('Heatmap for quantifiable phosphopeptides')
    exportgraphics(gcf,strcat(dir,'/Corr_heatmap_zscore_p',num2str(j),'.pdf'),'ContentType','vector')
end
close all
...
for j = 1:size(final_data_p_qnorm,2)
    ...
    %colormap(jet)
    colorbar('EastOutside')
    set(gca,'XTick',1:size(final_data_p_qnorm{j},2),'XTickLabels',header_p{j})
    xtickangle(45)
    ylabel('Phosphopeptide number')
    xlabel('Experiment ID')
    title('Heatmap for quantifiable phosphopeptides')
...
%Create the intensity distribution of the z transformed data
for j = 1:size(final_data_p_zscore,2)
    figure
    WindowAPI(gcf,'maximize')
    % violinplot(int_data)
    %al_goodplot(int_data)
    aa = violinplot(final_data_p_zscore{j});
    for i = 1:length(aa)
        aa(i).ViolinColor = Colors(i,:);
    end
    ylabel('log_2(Intensity)')
    ...
    grid on
    grid minor
    exportgraphics(gcf,strcat(dir,'/Violin_zscore_p',num2str(j),'.pdf'),'ContentType','vector')
end
close all
...
for j = 1:size(final_data_p_qnorm,2)
    ...
    % violinplot(int_data)
    %al_goodplot(int_data)
    aa = violinplot(final_data_p_qnorm{j});
    for i = 1:length(aa)
        aa(i).ViolinColor = Colors(i,:);
    end
    ylabel('log_2(Intensity)')
